...
export default function NewJobBar() {
  ...
  const handleAdd = () => {
    ...
    setTitle(''); setClient(''); setAddress(''); setOpen(false)
  }
      ...
      {!open ? (
        ...
        <>
          ...
          <input value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Address"
            className="border border-line rounded-sm px-2.5 py-1.5 text-sm w-36 focus:border-safety" />
          <button onClick={handleAdd} className="bg-charcoal text-bone text-sm font-medium px-3 py-1.5 rounded-sm">
            Add
          </button>
          <button onClick={() => setOpen(false)} className="text-steel text-sm">Cancel</button>
        </>
      )}
    </div>
  )
